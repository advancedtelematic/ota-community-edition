...
Then, run `./scripts/genclient.sh <device_name>`. `<device_name>` can be anything you choose. 

`genclient.sh` assumes a QEMU client running on localhost. For step-by-step instructions on building a QEMU client, see the [ATS Garage QEMU client build instructions](https://docs.atsgarage.com/quickstarts/qemuvirtualbox.html), and use the credentials.zip generated by OTA CE (from the `ota.ce` directory) instead of downloading one from ATS Garage.

For non-QEMU clients, specify the device address and ssh port to copy the generated credentials over:

    export DEVICE_ADDR=<device_ip>
    ./scripts/genclient.sh <device_name> <ssh-port>
